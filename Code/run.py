...
if __name__ == "__main__":
    ...
    if args.planner == 'astar':
        ...
        if args.planner == 'rrt':
            planner.planning_env.visualize_map(plan=plan, tree_edges=planner.tree.get_edges_as_states(), planner='RRT', map=str(args.map[3]), ext_mode=args.ext_mode, goal_bias=str(args.goal_prob), step_size=step_size)
        elif args.planner == 'rrtstar':
            if args.k == 0:
                args.k = 'log'
            planner.planning_env.visualize_map(plan=plan, tree_edges=planner.tree.get_edges_as_states(), planner='RRTStar', map=str(args.map[3]), ext_mode=args.ext_mode, goal_bias=str(args.goal_prob), step_size=step_size, k=str(args.k))